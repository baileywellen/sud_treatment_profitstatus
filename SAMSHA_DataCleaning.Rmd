...
state_control_long <- state_control_long %>%
                    ...
                    mutate(prev2years_bingedrinking = dplyr::lag(binge_drinking),
                           ...
                           druguse_calc = (cocaine_yr + heroin_yr), #combine the 3 drug use metrics - average 
...
state_govtUSE %>%
  group_by(year) %>%
  summarize(total = sum(count_tx, na.rm = TRUE))
```
...
state_final_df <- state_final_df %>% 
  left_join(cdc_overdoses, by = c("State_Abbrev" = "STATE", "year" = "YEAR")) %>%
  filter(!is.na(RATE)) %>%
  arrange(State_Abbrev, year) %>%
  group_by(State_Abbrev) %>%
  mutate(prevyear_deaths = dplyr::lag(RATE))
#state_final_df
