...
gender_df <- demographic_states %>%  
  ...
  mutate(Total = sum(across(where(is.numeric))),
         Percent = count / Total)  %>% 
   ungroup() %>%
  pivot_wider(id_cols = States, names_from = Gender, values_from = Percent) 
...
race_df <- demographic_states %>%  
  ...
  mutate(Total = sum(across(where(is.numeric))),
         Percent = count / Total)  %>% 
   ungroup() %>%
  pivot_wider(id_cols = States, names_from = Race, values_from = Percent) 
...
ethnicity_df <- demographic_states %>%  
  ...
  mutate(Total = sum(across(where(is.numeric))),
         Percent = count / Total)  %>% 
   ungroup() %>%
  pivot_wider(id_cols = States, names_from = Ethnicity, values_from = Percent) 
...
read_clean_insurance <- function(path, year_param)
  ...
  insurance_states <- insurance_states %>%
    mutate(Military = na_if(as.character(Military), "<.01")) %>% #change any "<.01" text to na
    mutate(Military = coalesce(as.numeric(Military), 1 - (Employer + Non.Group + Medicaid + Medicare + Uninsured))) #calculate the remaining percentage
...
freds_df <- read_excel("Data/StateYear_Income_Unemployed/State_Population_Unemployment_Income.xlsx", sheet=3) 

freds_df %>%
  t() %>%  #switch the rows and the columns so the values are in the rows and columns are years
  as.data.frame() %>% #cast to dataframe
  janitor::row_to_names(row_number = 1) %>% #move the first row to be the header
  tibble::rownames_to_column("freds_colname") %>% #rename the row names to "freds_colname"
  mutate(val = value_names[freds_colname]) %>%
  separate(val, into = c("variable_measure", "state_name"), sep = " in ") %>% #mark which variables we are looking at 
...
# STILL NEED TO CHANGE COLUMN NAMES, AND CREATE NEW VARIABLE COLUMN THAT COMBINES STATE NAME WITH THE VARIABLE MEASURE
