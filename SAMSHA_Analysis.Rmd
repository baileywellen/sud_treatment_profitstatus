...
more_controls_formula <- formula("count_tx ~ prev2years_aud + prev2years_drugusecalc + 
                                ...
                                scale(Race_White) +
                                (1| State_Name) + (1 | year)")
# summary(glmer.nb(count_tx ~ 
#                                 prev2years_aud + prev2years_drugusecalc + 
#                                 #prev2years_cocaine + prev2years_heroin +
#                                 scale(pop_millions) + 
#                                 scale(Income) +
#                                 scale(medicaid_enroll_percapita) +
#                                 scale(Gender_Male) +
...
#                                 scale(Unemployment) + 
#                                 scale(percapita_health_spending_thousands) +
#                                 scale(Insurance_Uninsured) + #may want to remove because of correlations
#                                 #no drug policy data
#                                 #no crime rates 
...
summary(glmer.nb(more_controls_formula, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e6)),
                               data = state_forprofit))
```
```{r}
log(state_forprofit$pop_millions)
```
