...
```{r}
library(corrplot)
corr_matrix <- cor(state_forprofit[, c("pop_millions", "percapita_health_spending_thousands", "medicaid_enroll_percapita", "prev2years_drugusecalc", "prev2years_aud", "prev2years_marijuana", "Gender_Male", "Ethnicity_HispanicorLatino", "Race_White", "Insurance_Uninsured", "Unemployment", "Income")])
...
tbl_summary(state_totals, include = c("count_tx", "pop_millions", "percapita_health_spending_thousands", "medicaid_enroll_percapita", "prev2years_drugusecalc", "prev2years_aud", "prev2years_marijuana", "Gender_Male", "Ethnicity_HispanicorLatino", "Race_White", "Insurance_Uninsured", "Unemployment", "Income")) %>%
...
glmer_nonprofit <- glmer.nb(more_controls_formula, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e6)),
                               data = state_nonprofit)
...
glmer_govt <- glmer.nb(more_controls_formula, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e6)),
                               data = state_govt)
...
```{r}
state_data_inc_alaska <- stateforprofit_randomeffects %>%
  mutate(fips = fips(State_Name))
...
glmer_all <- glmer.nb(more_controls_formula, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e6)),
                               data = state_totals)
